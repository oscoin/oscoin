...
module Test.Oscoin.Storage.Block.OrphanageNG
    ( tests
    ) where
...
import           Oscoin.Storage.Block.OrphanageNG
...
tests d = testGroup "Test.Oscoin.Storage.Block.OrphanageNG"
...
propPruneChainsShallow Dict = property $ do
    ...
    forAllShow gen (uncurry showChains) $ \(chain1, chain2) -> do
        ...
        conjoin [ map (reverse . toBlocksOldestFirst) firstBestCandidate
                                                       === Just (blocks' chain1)
...
bestChainOracle root xs = do
    ...
    pure $ fromChainSuffix blockScore (OldestFirst $ NonEmpty.fromList winningChain)

{------------------------------------------------------------------------------
  (Temporary) Orphans
------------------------------------------------------------------------------}

instance IsCrypto c => Condensed (ChainCandidate c tx s) where
    condensed ChainCandidate{..} =
           "Candidate { chain = "
        <> condensed ccChain
        <> ", score  = " <> condensed (mScore ccMeasure)
        <> ",rootHash = " <> condensed (mRoot ccMeasure)
        <> ",oldestHash = " <> condensed (mOldest ccMeasure)
        <> ",tipHash = " <> condensed (mTip ccMeasure)
        <> " }"
