...
import           Oscoin.Test.Storage.Block.SQLite (defaultGenesis)
    ...
    forAllShow (genOrphanChainsFrom forkParams initialChain) showIt $ \chains ->
        monadic runIdentity $ do
            ps <- forM chains $ \(orphanChain, missingLink) -> do
                ...
                let p1 = selectBestCandidate (blockHash defaultGenesis) orphanage' === Nothing
                -- Now we insert the missing link and we check the chain is
                -- what we expect.
                let orphanage'' = insertOrphans [missingLink] orphanage'
                let bestChain = reverse . toBlocksOldestFirst orphanage'' <$>
                      (selectBestCandidate (blockHash defaultGenesis) orphanage'')
    ...
    forAllShow (resize 3 $ shuffledOrphanChains forkParams initialChain) showIt $ \chains ->
            let finalOrphanage =
                  foldl' (\o (shuffledChain, _, missingLink) ->
                            insertOrphans (blocks shuffledChain)
                          . insertOrphans [missingLink]
                          $ o -- Order of insertion shouldn't matter!
                        ) orphanage chains
                bestChain =
                  selectBestCandidate (blockHash defaultGenesis) finalOrphanage
            in classifyChainsByScore (map firstAndLast chains) $
                 bestChain === (bestChainOracle (blockHash defaultGenesis) chains)

firstAndLast :: (a,b,c) -> (a,c)
firstAndLast (a,_,c) = (a,c)
...
propSelectBestCandidateComplex = do
    let orphanage = emptyOrphanage
        generator = do
            ...
            pure (chain, orph)
        showIt (chns, orph) = showOrphans . (chns,) . map firstAndLast $ orph
        forkParams = ForkParams 0 10 10  -- 10 fork of max 10 blocks.
    forAllShow generator showIt $ \(_initialChain, chains) ->
            let (finalOrphanage, allLinks) =
                ...
                bestChain lnk = selectBestCandidate (parentHash lnk) finalOrphanage
...
shuffledOrphanChains :: (Arbitrary tx, Arbitrary s, Serialise tx, Serialise s)
                     => ForkParams
                     -> Blockchain tx s
                     -> Gen [(Blockchain tx s, Blockchain tx s, Block tx s)]
shuffledOrphanChains params initialChain = do
    xs <- genOrphanChainsFrom params initialChain
    forM xs (\(c,b) -> (,c,b) . unsafeToBlockchain <$> shuffle (blocks c))
...
bestChainOracle :: BlockHash
                ...
                -> [(Blockchain tx s, Blockchain tx s, Block tx s)]
                -> Maybe ChainCandidate
...
instance Condensed ChainCandidate where
